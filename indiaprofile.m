...
col = 4; % cols
FSize=14
mm=0;nn=16;% nn=0太平洋 nn=8大西洋 nn=16印度洋
set(gcf,'color','white');
...
for i=1:row
    for j=1:col
        ...
        if mm==1
            ...
            xlast=pden(find(depth==90));
        end
        set(gca,'xlim',[OLF_xt-1 xlast],'xlim',[1021.5,1029.5],'XMinorTick','on','ydir','reverse',...
            'box','on','XAxisLocation','top')
        % ax = gca;
        % ax.YAxis.TickDirection = 'out';
        set(gca,'linewidth',2,'fontsize',FSize,'FontWeight','bold');
        if mm==1
            set(gca,'ylim',[0 600],'ytick',0:100:600);
        else if mm==2|mm==3
            set(gca,'ylim',[0 60],'ytick',0:10:60);
            else
                set(gca,'ylim',[40 100],'ytick',40:10:100);
            end
